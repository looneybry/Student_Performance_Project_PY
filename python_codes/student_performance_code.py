...
if df["Attendance_Rate (%)"].dtype == object:  # Check if the column is a string
    df["Attendance_Rate (%)"] = (
        df["Attendance_Rate (%)"].str.replace("%", "").astype(float)
    )
else:
...
study_vs_social_media = df[
    ["Study_Hours_per_Week", "Time_Spent_on_Social_Media (hours/week)"]
].corr()
...
learning_style_vs_exam_score = df.groupby("Preferred_Learning_Style")[
    "Exam_Score (%)"
].mean()
...
# Add a final newline to comply with PEP 8
